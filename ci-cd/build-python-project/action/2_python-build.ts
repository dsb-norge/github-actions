...
export async function run(): Promise<void> {
  ...
  try {
    ...
    // pre-type-check hook
    if (appVars['python-build-project-custom-command-pre-type-check']) {
      core.info('Executing pre type check custom command...')
      await executeCommand(appVars['python-build-project-custom-command-pre-type-check'], 'Custom command: pre type check')
    }
